...
Set in `src/agent.py`, the pipeline is **ASR → brain → TTS**: real-time speech-to-text, the brain is **Qwen** (`_build_llm`, via its OpenAI-compatible endpoint), and the voice is **MiniMax** (`_build_tts`). Configure with `QWEN_*` / `MINIMAX_API_KEY` in `.env.local`.
