...
- **`escalate_to_service()`** — builds the dossier from the session + instrument state and sends it as an **iMessage** via the Photon bridge (POSTs to the frontend's `/api/escalate` route, which runs the `spectrum-ts` SDK — Photon has no Python/REST send path; degrades to reading a summary aloud). The resolution path (`advance_step` with `fault_cleared`) also iMessages the tech a receipt.
